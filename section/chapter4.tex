...
\begin{enumerate}
\item Pada baris yang pertama yaitu untuk membuat suatu variabel cahyatest dari variabel cahyatrain dengan 450 data awal untuk dijadikan data training
\item Pada baris yang kedua yaitu untuk membuat variabel cahyatest dari variabel cahyatrain dengan 50 data yang sisanya untuk data pada bagian testing yang dimana dimulai dari data ke 451 sampai akhir data 
\end{enumerate}
\item Hasil Dari Pemecahan Dataframe
\begin{figure}[!hbtp]
\centering
\includegraphics[scale=0.3]{figures/cahyani2.jpg}
\caption{cahyani2}
\label{cahyani2}
\end{figure}
\end{itemize}
\par
\par
\item Vektorisasi Klasifikasi Data 
\par
\begin{lstlisting}
from sklearn import tree
...
clf = clf.fit(d_train_att, d_train_label)
\end{lstlisting}
\par Penjelasan tersebut sesuai dengan hasil.
\begin{enumerate}
\item yang dimana hasil dari impor tree dari sklearn. Dan untuk mendefinisikan variabel clf berfungsi untuk memanggil pada bagian Decission Tree Classifier serta melakukan fit atau suatu pengujian.
\item yang dimana akan menggunakan prediksi padabagian clf dengan menggunakan function predict untuk memprediksi suatu test. Dan hasilnya akan muncul dalam bentuk array.
\item pada bagian clf score akan memunculkan sebuah akurasi suatu prediksi
\end{enumerate}
\par
\begin{itemize}
\item Hasil Program Yang Menggunakan Decision Tree
\begin{figure}[!hbtp]
\centering
\includegraphics[scale=0.3]{figures/cahyani3.jpg}
\caption{cahyani3}
\label{cahyani3}
\end{figure}
\par
...
\begin{enumerate}
\item Pada hasil impor tree dari sklearn tersebut. Dan melauakan pendefinisikan variabel clf untuk memanggil dari Decission Tree Classifier dan melakukan sebuah fit atau suatu pengujian.
\item yang di dalamnya akan menjelaskan bahwa yang digunakan prediksi untuk clf dengan menggunakan function predict untuk memprediksi suatu test.
\item kemudian untuk yang clf score akan memunculkan akurasi prediksi yang dilakukan terhadap clf
\end{enumerate}
\begin{itemize}
\item Hasil Program Cross Validation
\begin{figure}[!hbtp]
\centering
\includegraphics[scale=0.3]{figures/cahyani5.jpg}
\caption{cahyani5}
\label{cahyani5}
\end{figure}
\par
...
\begin{itemize}
\item Pot Confusion Matrix yaitu fungsi dari code tersebut  untuk mencetak dan memplot Confussion Matrix. Yang dimana normalisasi dapat diterapkan dengan mengaturnya seperti `normalize = True`.
\item Hasil Plot Dari Confusion Matrix
\begin{figure}[!hbtp]
\centering
\includegraphics[scale=0.25]{figures/cahyani6.jpg}
\caption{cahyani6}
\label{cahyani6}
\end{figure}
\par
